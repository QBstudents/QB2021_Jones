...
author: "Joshua; Z620: Quantitative Biodiversity, Indiana University"
...
> Answer 1:
rnorm creates a normal distribution with mean and standard deviation defined by the inputs
the first value controls the number of values in the distribution
mean controls the mean fo the distribution
sd controls the standard deviation of the distribution
...
> Answer 2:
The original matrix (m) was 10c x 5r and the transposed matrix (n) is 5c x 10r
...
```{r}
meso.matrix <- meso[,3:8]
pairs(meso.matrix)
cor1 <- cor(meso.matrix)
```
...
> Answer 3:
There seems to be a positive correlation between total zooplankton biomass and total TIN, TP, TN, and SRP, also between total inorganic nutrient concentration and SRP, TN, and TP, and between SRP and TP and TN.
...
cor2 <- corr.test(meso.matrix, method = "pearson", adjust = "BH")
print(cor2, digits = 3)

cor3 <-corr.test(meso.matrix, method = "kendall", adjust = "BH")
print(cor3, digits = 3)
...
> Answer 4: 
Yes, results seem to be sensitive to whether the methods are parametric or non-parametric.
Non-parametric methods should be used whenever the researchers believe that the data may not be normally distributed.
No, there are minorly differeces in p-values when adjusting for false discovery rate but not to the point of changing any of the results from significance. 
Factoring in these false discovery rates is important because our uncorrected p-values give us the probability that our null hypothesis is being falsly rejected but as more hypotheses are tested the chance of falsly rejecting a null increases and has to be accounted for in order to correctly evaluate results.
...
ZPTNplot <- ggplot(data = meso, mapping = aes(y=ZP, x=TN))
ZPTNplot <- ZPTNplot + geom_point(aes(color = NUTS))
ZPTNplot <- ZPTNplot + labs(y = "Zooplankton Biomass (mg/L)", x = expression("Total Nitrogen"~mu~"/L"))
ZPTNplot <- ZPTNplot + geom_smooth(method = "lm", se = TRUE)
...
> Answer 5:
These results suggest that higher total nitrogren is positively correlated with higher total zooplankton biomass and that the difference between the medium and the low seems to be smaller than the difference between the medium and the high
...
zp.means <- as.data.frame(zp.means)
zp.means <- data.frame(zp.means, nuts.labels = c("Low","Medium","High"))
zp.means <- data.frame(zp.means, std.error = zp.sem)
zp.means$nuts.labels <- factor(zp.means$nuts.labels, levels = c("Low", "Medium", "High"))

barplot <- ggplot(data = zp.means, aes(x = nuts.labels, y = zp.means))
barplot <- barplot + geom_bar(stat = "identity", color = "black", position = position_dodge())
barplot <- barplot + geom_errorbar(aes(ymin = zp.means-std.error, ymax = zp.means+std.error, width=.2))
barplot <- barplot + theme_classic()
barplot <- barplot + labs(y = "Zooplankton Biomass (mg/L", x = "Nutrient Supply")
print(barplot)
...
```{r}
# Importing dataframe
zoops <- read.csv("data/zoops.txt", sep = "\t", header = TRUE)

#Removing Tank and NUTS
zoops <- zoops[,(3:11)]

#Adding column for total biomass (ZP)
zoops$ZP <- rowSums(zoops)

#Correlation searching
corr <- corr.test(zoops, method = "pearson", adjust = "BH")
print(corr)

corr2 <- corr.test(zoops, method = "kendall", adjust = "BH")
print(corr2)
# Correlation visualization
corr3 <- cor(zoops)

library(corrplot)
corrplot.mixed(corr = corr3, upper = "ellipse")

# Testing for significance taxa in determining total biomass
popanova <- aov(ZP ~ CAL + DIAP + CYCL + BOSM + SIMO + CERI + NAUP + DLUM + CHYD, data = zoops)
#popanova <- aov(ZP ~ CAL, data = zoops)
summary(popanova)

# Producing a data frame with data for a box plot
mean <- function(x){
  sum(x)/length(x)
}
zoops.means <- sapply(zoops, mean)
zoops.error <- sapply(zoops, sem)

bardata <- as.data.frame(zoops.means)
bardata <- data.frame(bardata, error = zoops.error)
bardata$taxa <- row.names(bardata)
print(bardata)

# Producing boxplot to visualize relative biomass
abundance.plot <- ggplot(data = bardata, aes(x = taxa, y = zoops.means, fill = taxa))
abundance.plot <- abundance.plot + geom_bar(stat = "identity")
abundance.plot <- abundance.plot + geom_errorbar(aes(ymin = zoops.means - error, ymax = zoops.means + error, width = .2))
adundance.plot <- abundance.plot + theme_classic()
abundance.plot <- abundance.plot + labs(y = expression("Average Zooplankton Biomass"~mu~"/L"), x = "Zooplankton Taxa")
print(abundance.plot)
```

> Answer: It seems as if the grand majority of the total zooplankton biomass comes from the Chydorus sp. but without the treatment information it isn't possible to draw conclusions on how different treatments affects this pattern. Additionally, I realized that since I calculated the total biomass using all of the taxa that they will obviously all be significantly correlated with it.
