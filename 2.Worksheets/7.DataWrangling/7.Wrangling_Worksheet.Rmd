...
Another popular approach for data wrangling is **`tidyverse`**, which is a collection of R packages.
...
#Making a string with all areas to be covered in the analysis
areas <- c(1:400)

#Making data frame to store results
species_area <- matrix(ncol = 2, nrow = 0)
species_area <- as.data.frame(species_area)
colnames(species_area) <- c("Area","Species")

#Running analysis for all areas from the plot
for (i in areas){
  
  #Randomly generate community with specified diversity
  com <- sim_poisson_community(s_pool = 100, n_sim = 1000, sad_type = "lnorm", sad_coef = list("meanlog" = 2,"sdlog" = 1), xrange = c(0,20), yrange = c(0,20))
  
  #Randomly placing plot into community of specified area to survey
  com_mat <- sample_quadrats(com, n_quadrats = 1, quadrat_area = i, method = "random", avoid_overlap = T)
  
  #Create dataframe to store area and species data
  temp <- as.data.frame(matrix(ncol = 2, nrow = 1)) 
  
  temp[,1] <- i
  temp[,2] <- specnumber(com_mat$spec_dat) 
  
  #Insert new data into dataset
  species_area[(nrow(species_area) + 1), ] <- temp
  if (i >= 400) {
    break
  }
}
lmZ1 <- lm(Area ~ Species, species_area)
anvZ1 <- anova(lmZ1)
summary(lmZ1)
```
> .6875
...
```{r}
library(ggplot2)
SAR1 <- ggplot(species_area, mapping=aes(x=Area, y=Species))
SAR1 <- SAR1 + geom_point()
SAR1 <- SAR1 + geom_smooth(method="lm")
print(SAR1)
```
...
#Making data frame to store results
species_area2 <- matrix(ncol = 2, nrow = 0)
species_area2 <- as.data.frame(species_area2)
colnames(species_area2) <- c("Area","Species")

#Running analysis for all areas from the plot
for (i in areas){
  
  #Randomly generate community with specified diversity
  com <- sim_thomas_community(s_pool = 100, n_sim = 1000, sad_type = "lnorm", sad_coef = list("meanlog" = 2,"sdlog" = 1), xrange = c(0,20), yrange = c(0,20))
  ...
  #Insert new data into dataset
  species_area2[(nrow(species_area2) + 1), ] <- temp
  if (i >= 400) {
    break
  }
}
lmZ2 <- lm(Area ~ Species, species_area2)
anvZ2 <- anova(lmZ2)
summary(lmZ2)
```
...
```{r}
SAR2 <- ggplot(species_area2, mapping=aes(x=Area, y=Species))
SAR2 <- SAR2 + geom_point()
SAR2 <- SAR2 + geom_smooth(method="lm")
print(SAR2)
```
...
```{r}
# Generating a figure showing both distributions
SAR1v2 <- ggplot(species_area, mapping=aes(x=Area, y=Species, color="blue"))
SAR1v2 <- SAR1v2 + geom_point()
SAR1v2 <- SAR1v2 + geom_smooth(method="lm")
SAR1v2 <- SAR1v2 + geom_point(species_area2, mapping=aes(x=Area, y=Species, color = "red"))
SAR1v2 <- SAR1v2 + geom_smooth(species_area2, mapping=aes(x=Area, y=Species, color = "red"), method="lm")
SAR1v2 <- SAR1v2 + scale_color_manual(values=c("#999999", "#E69F00"), 
                       name="Distribution",
                       labels=c("Poisson", "Thomas"))
print(SAR1v2)
```

> Both SARs show an increase in species detected as the area surveyed increases, with the Thomas showing less species in smaller plots and increasing at a steeper rate as sampling range increases. This makes sense as the Poisson has each species randomly distributed throughout the area with no relationship to conspecifics so the first plot will have a somewhat random number of species that increases throughout survey increases while Poisson has species in clusters meaning they inhabit less of the space so one small survey won't interact with as many different species than in the Poissan. 
In future simulations I think it would be interesting to have a simulations where some species can't inhabit the same space so we could see how species spread across space when there is a spacial resource that can be monopolized. Another interesting variable would be having variations in resources, or species prevalence across the area, as a way to simulate how species colonize habitats unequally based on niche space
...
Unless otherwise noted, this assignment is due on **Friday, April 9^th^, 2021 at 09:00 AM**.
